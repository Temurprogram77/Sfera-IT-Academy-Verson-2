...
const Attendance = () => {
  ...
  const summary = useMemo(() => {
    ...
    const kelgan = filteredData.filter(
      (r) => editingStatuses[r.id] === "Kelgan" || r.status === "Kelgan"
    ).length;
    const kelmagan = filteredData.filter(
      (r) => editingStatuses[r.id] === "Kelmagan" || r.status === "Kelmagan"
    ).length;
    const kechikkan = filteredData.filter(
      (r) => editingStatuses[r.id] === "Kechikkan" || r.status === "Kechikkan"
    ).length;
    ...
    datasets: [
      {
        ...
        data: filteredData.map((r) =>
          (editingStatuses[r.id] || r.status) === "Kelgan" ? 1 : 0
        ),
      ...
      },
      {
        label: "Kelmagan",
        data: filteredData.map((r) =>
          (editingStatuses[r.id] || r.status) === "Kelmagan" ? 1 : 0
        ),
        backgroundColor: "red",
      },
      {
        label: "Kechikkan",
        data: filteredData.map((r) =>
          (editingStatuses[r.id] || r.status) === "Kechikkan" ? 1 : 0
        ),
      ...
      <div className={`p-4 ${theme === "dark" ? "bg-gray-900" : "bg-white"}`}>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <Select
            ...
            value={filterGroup} // undefined bo'lsa placeholder ko'rinadi
          ...
          <Button
            type="primary"
            onClick={handleSave}
            disabled={Object.keys(editingStatuses).length === 0}
          >
            Saqlash
          </Button>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mt-4">
          <div className="p-4 rounded-lg shadow bg-gray-100 dark:bg-gray-800">
            ...
            <h2 className="text-xl font-bold">{summary.all}</h2>
          </div>
          <div className="p-4 rounded-lg shadow bg-green-100 dark:bg-green-800">
            <p>Kelganlar</p>
            <h2 className="text-xl font-bold">{summary.kelgan}</h2>
          </div>
          <div className="p-4 rounded-lg shadow bg-red-100 dark:bg-red-800">
            <p>Kelmaganlar</p>
            <h2 className="text-xl font-bold">{summary.kelmagan}</h2>
          </div>
          <div className="p-4 rounded-lg shadow bg-orange-100 dark:bg-orange-800">
            <p>Kechikkanlar</p>
            <h2 className="text-xl font-bold">{summary.kechikkan}</h2>
          </div>
        </div>
